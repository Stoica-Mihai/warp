...
/// Returns the canonical identifier for the application.
    ...
    /// Returns whether this build can ship telemetry events. This fork has no
    /// telemetry backend, so UI that controls telemetry stays hidden.
    pub fn is_telemetry_available() -> bool {
        false
    }

    /// Returns whether this build can ship crash reports. This fork has no crash
    /// reporting backend, so UI that controls crash reporting stays hidden.
