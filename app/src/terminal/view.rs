...
use session_sharing_protocol::common::{
    AgentAttachment, LongRunningCommandAgentInteractionState, ParticipantId, Role, ServerConversationToken as SessionSharingServerConversationToken,
...
impl TerminalView {
    ...
    fn try_navigate_to_parent_conversation(&mut self, _ctx: &mut ViewContext<Self>) -> bool {
        false
    }
    ...
    fn handle_usage_footer_toggled(
        ...
        let agent_view_conversation_id = None;
    ...
    pub fn active_conversation_id(&self, _app: &AppContext) -> Option<AIConversationId> {
        None
    }
    ...
    pub(crate) fn stop_local_agent_conversation(
        ...
        let visible_conversation_id = None;
    ...
    fn typed_characters_on_terminal(&mut self, characters: &str, ctx: &mut ViewContext<Self>) {
        ...
        if self.should_write_typed_chars_to_pty(ctx) {
            ...
            // Don't clear selected blocks/text in AI mode (context blocks).
    ...
    fn insert_shell_process_terminated_banner(
        ...
        if self.is_login_shell_bootstrapped {
            ...
            if let Some(_termination_reason) = termination_reason {
                let (_shell_path, _shell_type) = self.get_shell_starter_local(ctx).unzip();
                let _antivirus_name = AntivirusInfo::as_ref(ctx).get();

                let _long_os_version = crate::system::long_os_version(ctx);
            };
    ...
    pub(crate) fn update_agent_view_back_button_state(&mut self, _ctx: &mut ViewContext<Self>) {}
    ...
    fn maybe_auto_open_cli_agent_rich_input(&mut self, _ctx: &mut ViewContext<Self>) {}
    ...
    fn enter_environment_setup_selector(&mut self, args: Vec<String>, ctx: &mut ViewContext<Self>) {
        ...
        // No arguments provided - show the mode selector modal
    ...
    /// Returns the conversation id targeted by the pending user query block, if any.
    fn pending_user_query_conversation_id(&self) -> Option<AIConversationId> {
        None
    }

    /// Removes the pending user query rich content block, if present, and clears its tracking state.
    fn remove_pending_user_query_block(&mut self, _ctx: &mut ViewContext<Self>) {
        self.pending_user_query_kind = None;
        let Some(view_id) = self.pending_user_query_view_id.take() else {
            return;
        };
        let mut model = self.model.lock();
        model.block_list_mut().remove_rich_content(view_id);
...
impl TerminalView {
    ...
    fn is_last_block_init_step(&self, _ctx: &AppContext) -> bool {
        self.rich_content_views
            .last()
            .is_some_and(|rc| rc.is_init_step())
    }
    ...
    fn active_init_environment_block(
        ...
        let last_visible_block = self.rich_content_views.last()?;
    ...
    pub fn redetermine_global_focus(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let should_focus_terminal = {
            ...
            let has_block_or_text_selection_in_shell_mode =
                is_shell_mode && (are_blocks_selected || is_text_selected);
