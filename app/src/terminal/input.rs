...
use crate::workflows::info_box::{WorkflowsMoreInfoView, WORKFLOW_PARAMETER_HIGHLIGHT_COLOR};
...
impl Input {
    ...
    pub fn on_execute_command_for_shared_session_participant_failure(
        ...
        let Some(ViewerCommandExecutionRequest) = shared_session_input_state
    ...
    fn should_restore_buffer_on_inline_menu_dismiss(&self, _ctx: &ViewContext<Self>) -> bool {
        true
    }
    ...
    pub fn generate_autosuggestion_async(
        ...
        let reverse_chronological_potential_autosuggestions: Option<Vec<HistoryEntry>> = {
            let history_entries = History::as_ref(ctx).commands(session_id);
...
fn maybe_render_ai_input_indicators(
    ...
    // CLI agent rich input (e.g. Claude Code bash mode).
