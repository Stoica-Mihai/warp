...
use crate::resource_center::{
    mark_feature_used_and_write_to_user_defaults, skip_tips_and_write_to_user_defaults, Tip,
...
impl Workspace {
    ...
    fn has_right_region(&self, app: &AppContext) -> bool {
        self.active_tab_pane_group().as_ref(app).right_panel_open
    }
    ...
    fn send_feedback(&mut self, ctx: &mut ViewContext<Self>) {
        // Warp-AI feedback skill removed; always open the feedback form URL.
    ...
    pub fn toggle_tab_bar_overflow_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let menu_items: Vec<MenuItem<WorkspaceAction>> = vec![];
    ...
    fn set_focused_index(&mut self, _index: Option<usize>, ctx: &mut ViewContext<Self>) {
        ctx.notify();
    }
                ...
                if self.active_tab_pane_group().as_ref(ctx).left_panel_open && is_target_active {
                    ...
                    self.left_panel_view.update(ctx, |left_panel, ctx| {
                        let action = match target_view {
                            LeftPanelTargetView::FileTree => Some(LeftPanelAction::ProjectExplorer),
                            LeftPanelTargetView::WarpDrive => None,
                        };
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            UndoTrash(cloud_object_type_and_id) => {
                let _ = cloud_object_type_and_id;
            }
            TerminateApp => {
