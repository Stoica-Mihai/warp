...
use crate::util::git::{
    create_pr, get_file_change_entries, run_commit, run_push,
...
pub(super) fn new_state(
    ...
    // Warp AI commit-message generation removed; always the manual-type prompt.
    ...
    ctx.spawn(
        async move {
            ...
            let outcome = match intent {
                ...
                CommitIntent::CommitAndCreatePr => {
                    run_push(&repo_path, &branch_name, path_env_ref).await?;
                    // `gh pr create --fill` — Warp AI title/body generation removed.
