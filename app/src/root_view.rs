...
const WINDOW_TITLE: &str = "Sublight";
...
/// Border drawn around the whole window. Hard-coded for the default Dark theme.
...
fn create_environment(arg: &CreateEnvironmentArg, ctx: &mut AppContext) {
    ...
    root_handle.update(ctx, |root_view, ctx| {
        let workspace_handle = &root_view.workspace;
        {
...
impl RootView {
    ...
    fn traffic_light_data(&self, _ctx: &AppContext) -> Option<TrafficLightData> {
        // Workspace view handles traffic light rendering (always in terminal state).
        None
    }
}
