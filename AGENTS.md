...
**CRITICAL (session 40): Dead code deletion scope — Warp/AI-specific ONLY.** Dead code appearing in all 3 gates must still NOT be deleted if it lives in a generic local-feature module. It may be dead because a feature flag was removed; keep it for potential future re-enablement.
- **Generic modules — keep dead code even if 3-gate dead:** `code/editor/`, `code_review/`, code diff viewer, terminal UI (`terminal/input/`, view components), MCP (`ai/mcp/`), CLI agent sessions plugin manager (`terminal/cli_agent_sessions/plugin_manager/`), workspace view helpers, UI components (`view_components/`, `ui_components/`).
- **Warp/AI-specific modules — delete dead code:** `server_api/ai.rs`, `ai/ambient_agents/`, `ai/execution_profiles.rs`, `ai/request_usage_model.rs`, cloud handoff clusters, AI billing/credits structs, AI orchestration types (hidden child agent conversations), AI agent action converters (`ai::agent::action::`-dependent).
- **Classification test:** If the dead item's imports or type dependencies touch `ai::agent::action`, `server_api::ai::AIClient`, Warp cloud API calls, or Warp billing/credits → Warp-AI-specific, safe to delete. If it only touches UI state, editor models, MCP server management, or generic terminal UI → generic, keep it.
...
**NEXT: Continue FeatureFlag sweep**:
Remaining AI/cloud flags. Key targets:
- AI agent API flags (ConfigurableContextWindow, LinkedCodeBlocks, ReadImageFiles, V4AFileDiffs, RunAgentsTool, MCPGroupedServerContext, PromptSuggestionsViaMAA, AskUserQuestion) — all in `ai/agent/api/` files
- AgentModeComputerUse + LocalComputerUse (4 callers each)
- SoloUserByok (3 callers in user_workspaces.rs)
- RemoteCodebaseIndexing (17 callers)
See session 28 handoff at `/tmp/session28-handoff.md`.
