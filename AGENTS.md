...
**NEXT (session 57):** `ai/harness_availability.rs` (11.1K) — harness availability. Small Warp-AI TypedActionViews: `AgentTodosPopupView`, `DeleteConversationConfirmationDialog`. Larger: `drive/` + `cloud_object/` Warp Drive cluster (21K+, 144 callers — note `JsonObjectType` lives in the `warp_server_client` crate, outside `-p warp`). See plan.md "AI strip — current state" for full handoff.
