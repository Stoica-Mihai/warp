...
**Next**: firebase + auth/server layer strip (the big remaining cloud surface). Other parked items: full Tier-C library-crate rename (`warp_*` → `sublight_*`), `Channel` enum variant cascade, privacy/auth UI consolidation, broader autoupdate strip (3680 LoC across 8 files, but high callgraph fan-in so needs careful sequencing).
