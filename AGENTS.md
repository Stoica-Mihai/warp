...
**Next**: firebase + auth/server layer strip (the big remaining cloud surface). Other parked items: full Tier-C library-crate rename (`warp_*` → `sublight_*`), broader privacy/auth UI consolidation, channel-bin cleanup (stable/preview/dev/local/integration).
