...
**Next**: firebase + auth/server layer strip (login pass — drops `crates/firebase`, `server_api/auth.rs` token-fetch surface, AuthView UI). Followups still parked: `app/Cargo.toml` `agent_onboarding` / `hoa_onboarding_flow` feature flags and `assets/onboarding/` bundles (orthogonal cleanup); README rebrand.
