...
**NEXT (session 57):** Clean the +10 exposed `ai/execution_profiles.rs` dead types (AIExecutionProfile, CloudAIExecutionProfile, AIExecutionProfileInfo, RunAgentsPermission, AskUserQuestionPermission) — used only by dead persistence/sqlite.rs + workspaces/gql_convert.rs paths (touches persistence). Then `ai/harness_availability.rs` (11.1K). Small Warp-AI TypedActionViews: `AgentTodosPopupView`, `DeleteConversationConfirmationDialog`. See plan.md "AI strip — current state" for full handoff.
