...
1. **AIClient** (safest, AI already gutted) — remove consumers feature-by-feature then trait+impl+get_ai_client+the gql ops. PROGRESS:
   ...
   - **(1b NEXT)** code_review AI (generate_code_review_content) in code_review/git_dialog commit.rs/pr.rs + comment_list_view AI buttons. REST call (not gql).
   - **terminal/workspace AI gates** — the `has_any_ai_remaining` consumers (terminal input/UDI, workspace/mod, code_review comment_list).
   - **(1a LAST, not first — it's the GATE)** AIRequestUsageModel: the model's `has_any_ai_remaining`/`has_requests_remaining`/quota methods gate the AI features above, so it can only go AFTER them. Removing it: drop the server polling (refresh_request_usage_async/get_request_limit_info trait method/update_request_limit_info/cache_request_limit_info — all dead since 1c) + the `ai_client` field + ctor param (cascades to lib.rs:1127 + ~8 test inits) + GetRequestLimitInfo gql op + the whole model once has_any_ai_remaining consumers are gone. settings/ai.rs RequestLimitInfo quota display also goes.
   - **(1e)** delete AIClient trait/impl/get_ai_client once all 3 methods gone.
