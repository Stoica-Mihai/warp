...
1. **AIClient** (safest, AI already gutted) — remove consumers feature-by-feature then trait+impl+get_ai_client+the gql ops. PROGRESS:
   ...
   - **(1a NEXT — DECISION MADE: REMOVE ALL WARP-AI SURFACES)** User chose full removal (no Warp agent at all; vendor CLI agents as shell processes are untouched). So this is a multi-increment epic, not one cut. Scope (each its own green increment; do the render/input-path ones with a GUI build + eyeball, cargo-green ≠ works):
     ...
     - **(1a-iv NEXT — full AI-usage/quota/bonus subsystem deletion, WIDE cascade)** All behavioral has_any_ai_remaining consumers are gone; only `comment_list_view::debug_state` (a debug snapshot with ai_available/ai_enabled fields → cascade to right_panel.rs:1409-1410 debug log + CommentListDebugState struct) still calls it. Deleting `ai/request_usage_model.rs` (AIRequestUsageModel + RequestLimitInfo + RequestUsageInfo + BonusGrant + BonusGrantScope + RequestLimitRefreshDuration + has_any_ai_remaining/has_requests_remaining/bonus-credit methods) cascades to: **settings/ai.rs (29 refs — the whole quota-reset-banner subsystem: was_quota_exceeded/QuotaBannerState/update_quota_info)**; **workspaces/gql_convert.rs (BonusGrant::from_gql_bonus_grant + BonusGrantScope + the bonus-grants-purchased conversion)**; lib.rs:203/1127 (import + registration + the ai_client local); test_util/terminal.rs + request_usage_model_tests.rs (~25 tests) + settings/ai_tests.rs; server_api/ai.rs get_request_limit_info (the AIClient last method). This is a subsystem removal, do as a focused increment.
