...
- (5) ⬜ CRATES-ONLY (deferred — intricate, ~0 binary): managed_secrets 2 Bedrock ManagedSecretValue variants (AnthropicBedrockAccessKey/ApiKey + constructors + secret_value_tests) + the manager.rs:89-99 mapping arms; warp_cli secret.rs Bedrock subcommands (BedrockApiKey/BedrockAccessKey + SecretType::AnthropicBedrockApiKey) + agent.rs bedrock_inference_role/bedrock_role_region args + lib_tests. **ENTANGLED:** crates/graphql task_secrets.rs ManagedSecretAnthropicBedrock* + managed_secrets.rs ManagedSecretType Bedrock variants are SCHEMA-BOUND (cynic) → KEEP; the manager must then drop/ignore those wire secret-types when building ManagedSecretValue. Gate incl `-p warp_cli`. KEEP ManagedSecretValue type + ManagedSecretManager + issue_task_identity_token (all generic/MCP).
