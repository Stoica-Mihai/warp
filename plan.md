...
1. **AIClient** (safest, AI already gutted) — remove consumers feature-by-feature then trait+impl+get_ai_client+the gql ops. PROGRESS:
   ...
   - **(1a NEXT — DECISION MADE: REMOVE ALL WARP-AI SURFACES)** User chose full removal (no Warp agent at all; vendor CLI agents as shell processes are untouched). So this is a multi-increment epic, not one cut. Scope (each its own green increment; do the render/input-path ones with a GUI build + eyeball, cargo-green ≠ works):
     - **(1a-i, bounded, safe)** workspace/mod `is_feedback_skill_available` → drop the `has_any_ai_remaining` (and `is_any_ai_enabled`?) conjunct → SendFeedback always opens the GitHub issue form. Removes 1 of 3 has_any_ai_remaining consumers.
     - **(1a-ii, bounded)** code_review/comment_list_view: remove the `ReviewDestination::Warp` (send-comments-to-Warp-agent) path → code review sends only to CLI agents (`ReviewDestination::Cli`). Drops `ai_available`/has_any_ai_remaining + the Warp-agent destination UI. Removes 2nd consumer.
     - **(1a-iii, BIG, render/input path — GUI-verify)** terminal agent-mode AI input: terminal/input.rs:2233 + universal_developer_input.rs:363 + the agent-mode input surface. This is the large remaining Warp-AI UI; thousands of lines; the input/render path. Plan it fresh, build the GUI, eyeball.
     - **(1a-iv)** once all 3 has_any_ai_remaining consumers gone: delete AIRequestUsageModel entirely (model + polling refresh_request_usage_async/get_request_limit_info/update/cache + ai_client field/ctor [cascade lib.rs:1127 + ~8 test inits] + GetRequestLimitInfo gql op + settings/ai.rs RequestLimitInfo + ~25 model tests) + AISettings AI gating (is_any_ai_enabled etc., as far as it unwinds).
   - **(1e)** delete AIClient trait/impl/get_ai_client once get_request_limit_info goes (1a-iv).
