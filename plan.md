...
- [~] **R5 batch-2b** — the 5 HIGH-FANOUT guarded flags (each on-by-default → gated code is currently live; collapse to off = real feature removal). 1 of 5 done:
  ...
  - [ ] **DiffSetAsContext(7)** — DEEP CASCADE (editor-coupled): "add diff set as AI context" woven through code_review header buttons + file-row `add_context_button` + the code-EDITOR element's per-hunk `add_hunk_as_context_button`/`AddAsContextButton`/`set_add_diff_hunk_as_context_button` + context.rs (`convert_file_diffs_to_diffset_hunks`/`DiffSetHunk`) + insert_diff_as_context + `CodeReviewAction::AddDiffSetAsContext`/`DiffSetScope`. Must determine if the editor add-context-button is AI-only before cutting (risks the KEPT code editor). Own focused effort.
