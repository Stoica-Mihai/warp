...
- **AMBIENT AGENTS strip — IN PROGRESS (session 67).** Warp cloud scheduled/cron AI agents. **Creation is RUNTIME-DEAD** (every gate hardwired false/None: `is_ambient_agent_session()→false`, `ambient_agent_task_id()→None`, `SessionSourceType::AmbientAgent` 0 constructors, `PanesLayout::AmbientAgent` never built). DONE: `github_auth_notifier` (`f19dcccd`) + terminal_pane snapshot collapse. **REMAINING (full file:line map in subagent output / do as prepared atomic commits):**
  - **(data layer, medium)** Delete `app/src/ai/cloud_environments/` + `ai/ambient_agents/scheduled.rs` (ScheduledAmbientAgent) + `JsonObjectType::{ScheduledAmbientAgent,CloudEnvironment,CloudAgentConfig}` variants in `crates/warp_server_client/src/cloud_object/mod.rs` (hand-written enum, NOT cynic — safe delete, TryFrom already Errs on unknown) + sqlite.rs Cloud-model load arms (`:50,54,2318-2349`). SNAG: `context_chips/display_menu.rs` `ChipMenuType::Environments` (~25 dead render arms + EnvironmentSidecarData + the `CloudAmbientAgentEnvironment::get_by_id` at `:643`) — never constructed (only a self `==` check at `:623`); sizable isolated display cleanup, collapse `should_show_environment_sidecar→false` first.
