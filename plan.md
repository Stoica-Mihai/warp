...
- [ ] **R5 batch-2** — the 13 flags WITH real `.is_enabled()` guards (need per-flag guard-collapse to the always-on/off behavior, then remove): AgentModeAnalytics(1: privacy.rs), CustomInferenceEndpointsEnterprise(1: user_workspaces), AIContextMenuCode(2), ProfilesDesignRevamp(3), QueueSlashCommand(3), APIKeyAuthentication(2), ArtifactCommand(3), AgentModeWorkflows(3), OrchestrationV2(7), DiffSetAsContext(7), SharedWithMe(12), AgentView(17), AgentMode(24). The big ones (AgentMode 24, AgentView 17, SharedWithMe 12) are mini-projects — collapse each guard, verify the gated feature is truly dead. HOANotifications/OpenCode/Codex/Gemini notification flags = vendor-CLI infra, KEEP.
