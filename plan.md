...
1. **AIClient** (safest, AI already gutted) — remove consumers feature-by-feature then trait+impl+get_ai_client+the gql ops. PROGRESS:
   ...
   - **(1a NEXT — DECISION POINT, not mechanical)** AIRequestUsageModel. `has_any_ai_remaining` is NOT a simple delete — it encodes real AI-availability logic (server credits **and** a BYOK-key path) and feeds STILL-LIVE surfaces: workspace/mod `is_feedback_skill_available` (feedback skill in agent pane), code_review/comment_list_view `render_send_button`/`debug_state` (send-comments-to-Warp-agent enable+tooltip), terminal/input.rs:2233 + universal_developer_input.rs:363 (terminal AI input). **DECISION NEEDED:** does Warp-AI-via-BYOK stay (collapse has_any_ai_remaining to its BYOK-only branch, keep terminal AI input + send-to-agent) OR are ALL Warp-AI surfaces removed too (terminal AI input, code-review send-to-agent, feedback-skill-via-AI)? Only after that: drop the model's server polling (refresh_request_usage_async/get_request_limit_info/update/cache — all dead since 1c) + `ai_client` field + ctor param (cascade lib.rs:1127 + ~8 test inits) + GetRequestLimitInfo gql op + settings/ai.rs RequestLimitInfo quota display + the request_usage_model_tests (~25 has_any_ai_remaining tests).
   - **(1e)** delete AIClient trait/impl/get_ai_client once get_request_limit_info goes with 1a.
