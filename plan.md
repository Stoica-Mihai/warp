...
**S67: Firebase anonymous-auth dead-branch collapse + auth_state dead-code sweep.** Two commits:
- `ed753c93` — collapsed always-false `is_user_anonymous()` branch in `initiate_user_signup` (Firebase anon auth stripped → branch permanently false); deleted `initiate_anonymous_user_linking` + `open_url_maybe_with_anonymous_token` + `URLConstructorCallback` stubs from `auth_manager.rs`; inlined `data_management_url(None)` at `privacy_page.rs` call site. −30 LoC.
- `2e21d6b2` — deleted 0-caller auth_state methods: `is_anonymous_user_feature_gated`, `is_anonymous_user_past_object_limit`, `anonymous_user_renotification_block_expired`, `anonymous_user_type`, `personal_object_limits`, `new_anonymous_for_test`. Removed now-unused imports: `ObjectType`, `PersonalObjectLimits`, `chrono::{DateTime,Duration,Utc}`; dropped `ANONYMOUS_USER_NOTIFICATION_BLOCK_TIMER` constant. −90 LoC.
...
- **De-brand rename** (`warp*` → `sublight*`) — binary name, crate identifiers, warp.dev references in UI strings. Plan marks this "LAST, after cloud surface gone." **Cloud surface is now gone. This is the primary next step.** Scope: rename `sublight` binary (already done in build config?), sweep `warp_*` crate names and `use warp_*` imports, replace remaining `warp.dev` / `warpify` string literals in UI, update `Cargo.toml` workspace member names. Risk: mechanical fan-out across many files, no behavior change. Approach: recast + batch rename. Verify with `cargo check` after each crate rename.
