...
- **AMBIENT AGENTS strip — ✅ COMPLETE (session 67).** Warp cloud scheduled/cron AI agents, fully removed across 4 commits: `github_auth_notifier` (`f19dcccd`); ambient cloud DATA LAYER (`7152c906`, −0.26 MB) — scheduled/task/JsonObjectType::{ScheduledAmbientAgent,CloudAgentConfig}; Environments context-chip MENU + cloud_environments (`ba6eb805`, −0.38 MB); **AmbientAgentTaskId KEYSTONE (`dc2f00db`, −0.23 MB)** — `ai/ambient_agents/` + `LeafContents::AmbientAgent`/`AmbientAgentPaneSnapshot` + persistence + pane-creation chain + `PaneMode::Cloud`/`TabConfigPaneType::Cloud` + terminal_model `ambient_agent_task_id()`/`ViewingAmbientConversation` + dead workspace actions + handlers + obsolete tests. **KEPT (not keystone, harmless dead — separate cosmetic pass):** `is_ambient_agent_session()→false`, `is_shared_ambient_agent_session`, `SessionSourceType::AmbientAgent` (wire), `Indicator::AmbientAgent`, `AddAmbientAgentTab`, `CancelAmbientAgentTask`, the cloud-mode-view scaffolding (`terminal/view/ambient_agent/` + `enter_cloud_agent_view`/`enter_cloud_mode_from_session`), `ambient_agent_panes` sqlite table+migration. **KEPT (Frontier-B, not ambient):** `open_cloud_conversation_from_server_token`, ServerConversationToken/AIConversationId paths.
