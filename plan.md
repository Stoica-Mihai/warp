...
**S69: unambiguous Warp-AI vestige removal** (6 commits `b17eac19`→`6fce08a3`, −0.20 MB). Applied the sharpened rule (generic-kept only if a generic/vendor-CLI feature consumes it): deleted Warp-AI-specific dead code — BYO-LLM api_keys module, WarpAiExecutionContext, OpenAgentProfileEditor chain + cloud-agent computer-use stub, SummarizeAIConversation + StartNewAgentConversation actions, CLISubagentController stub (relocated its 2 live vendor-CLI serde enums to interaction_mode.rs). Audit confirmed NO live Warp-AI (no AIClient/agent-send/transport) + NO native auto-egress. KEPT generic/vendor-CLI consumers (skills, code_review, MCP, autonomy-permission enums, conversation status). DEFERRED then DONE: dead override_* server-URL plumbing (`62cae227`) + AddAmbientAgentTab/cloud-agent-tab + URI deep-link (`5b0648cc`, kept persisted CloudAgent variant). STILL DEFERRED: (B) AIExecutionProfilesModel singleton — blocked by already-broken out-of-3-gate integration_tests step.rs, low value, skip/own-pass; (D) settings/ai.rs split — ✅ DONE (5 commits `ed847f18`→`38a9fa60`, −2.80 MB → 604.7 MB; ~30 dead Warp-agent fields removed in 5 batches; serde-safe key-tolerant loader; KEPT vendor-CLI/MCP/skills/NLD-autosuggestion settings). Only (B) AIExecutionProfilesModel remains (integration-gate, low value). Detail in memory `ai-strip-session-69-ai-vestige`.

**S68: wasm / Warp-for-Web browser build target fully abandoned** (6 commits `fb1d6e33`→`fcb83921`, binary flat — linker-invisible). The wasm target compiled the whole app to run in a browser tab (the app.warp.dev hosted terminal); its `uri/web_intent_parser` + `browser_url_handler` were the **web viewer for shared sessions / AI conversations / Drive objects** — cloud surfaces already stripped on the desktop side. Removed: `script/wasm/` + release-wasm/dev-wasm cargo profiles + `.cargo/config.toml` wasm rustflags (`fb1d6e33`); webapp app modules + WebIntent surface + inert refs (`728e9210`+`eba6745e`); crate-level wasm modules in warpui/warpui_core/websocket/ipc/warp_logging (`ec92bb92`); all `cfg(wasm)` Cargo dep blocks + workspace wasm deps wasm-bindgen/web-sys/gloo (`c44f9abe`). wasm build now structurally impossible. Native desktop (Linux/macOS/**Windows** — all `not(wasm)`) unaffected. **Bucket 5 (inert cfg(wasm) annotation purge) — ✅ COMPLETE** (`737ade0f`…`36436c14`, 8 phases, binary flat/linker-invisible): 730→37 wasm cfg lines (95% removed) — stripped all `cfg(not(wasm))` gates→unconditional, all positive `cfg(wasm)` items/blocks/cfg_if-arms/cfg_attr/`cfg!`→false, simplified compound `not(wasm)` cfgs. The 37 kept are native-live guards (not(wasm) runtime checks, real multi-platform compounds, build.rs cfg aliases, searcher Tantivy whole-file gate, lib.rs Worker-on-wasm panic). wasm/Warp-for-Web is now fully gone from the tree.** KEPT: `instant` crate (native timing dep, its wasm-bindgen feature is inert on native).

**S67: referrals / reward-theme modal feature stripped** (`3909f1fd`, −0.21 MB). Removed Warp-proprietary referral-reward subsystem: `reward_view.rs` + `referral_theme_status.rs`; ThemeKind::{Sent,Received}ReferralReward variants + builders; all workspace wiring (reward_modal field/subscribe/show_reward_modal/handle_referral_theme_status_event/render branch/is_reward_modal_open); ThemeChooser referral param; GlobalResourceHandles.referral_theme_status; ContextFlag::ShowRewardModal. 12 files, −562 LoC. Migration-safe (error-tolerant theme loader → default fallback). **Cloud-surface strip now functionally complete. PRIMARY NEXT = de-brand rename (`warp*` → `sublight*`).**
...
- `2e21d6b2` — deleted 0-caller auth_state methods (`is_anonymous_user_feature_gated`, `is_anonymous_user_past_object_limit`, etc.) + dead imports + constant. −90 LoC.
- `ea28491f` — docs update.
- `<s67-phone-home-1>` — stripped warp.dev phone-home URLs (auth sign-up/sign-in/redirect, billing upgrade links, staging-server check, AI conversation links): deleted `ChannelState::uses_staging_server()`, `ServerConversationToken::debug_link/conversation_link`, `AuthManager::sign_up_url/sign_in_url/generate_auth_state/pending_auth_state`, `UserWorkspaces::upgrade_link/upgrade_link_for_team`; collapsed `initiate_user_signup`/`redirect_to_sign_in`/`Reauth`/`ShowUpgrade`/`OpenDataManagementWebpage` handlers to no-ops; deleted `render_error_header_with_upgrade_link` + `OpenUpgradeLink`/`AttemptLoginGatedUpgrade` from CommandSearchAction. −257 LoC.
- `<s67-phone-home-2>` — deleted `font_fallback.rs` (1436 LoC, wasm-only, fetched fonts from app.warp.dev CDN at runtime). −1441 LoC.
- `<s67-phone-home-3>` — deleted oz-platform SKILL.md (Warp cloud agent platform skill referencing app.warp.dev/api/v1/agent/*); removed `warp_server_url` template variable from `build_bundled_skill_context()`; collapsed `OpenDataManagementWebpage` to no-op + deleted `data_management_url()`. −517 LoC.
- `<s67-phone-home-4>` — removed last desktop warp.dev URL infrastructure: deleted `allows_server_url_overrides()` CLI override block (always-false), deleted `set_before_open_url` callback (rewrote warp.dev URLs to native intents), gated `web_intent_parser` module as wasm-only (all remaining callers are wasm). `server_root_url()` now only used in config definition + wasm-gated code. −36 LoC.

**S62–63: GraphQL / cloud-transport removal — COMPLETE.** `warp_graphql` + `warp_graphql_schema` crates DELETED (`145f7aea`), all 4 gql client traits gone (AIClient, ManagedSecretsClient, TeamClient/workspaces-fetch, the ServerApi gql egress). Session 63 specifics (full detail: memory `ai-strip-session-63-graphql-crate-deletion.md`): Inc 0–5 dead-consumer removal · Inc 6 ServerApi egress+event-system teardown (1394→105 LoC) · Inc 7 `ServerTimestamp` re-homed to `warp_util` · Inc 8b warp_server_client Server* model + gql-convert layer stripped (−1758 LoC) + ConflictStatus collapsed · Inc 9 managed_secrets reduced to ManagedSecretValue (serde-only) · Inc 9b dead Workspace billing removed · **Inc 10 the 2 gql crates deleted (−8.91 MB — cynic schema + op-builders freed at crate deletion)**. Net session: 639.4 → 629.7 MB. KEY LESSONS folded into the `remove-feature` skill (grep-all-roots, stale-cache false-green, schema-types-die-with-crate, framework-is-a-kept-feature's-store, payoff-at-crate-deletion, re-home-to-shared-lower-crate, usable-vs-present diagnostic).

**S64: cloud_object FEATURE removal CR1–CR3 DONE.** Decision: keep ONLY **LocalWorkflows** (genuinely usable: `ctrl-r`/`input:toggle_workflows` → built-in + `~/.warp/workflows` YAML → insert command); REMOVE the 4 create-gated-dead/broken Drive features. **CORRECTIVE: warp_server_client + cloud_object framework STAY — MCP servers persist through them.** CR1 (`0123abe3`, ~0 MB) · CR2 (`0cac96ce`, −0.07 MB) · **CR3 (`c9494999`, −5.59 MB)** Env-Var Collections. CR4–CR8 remain (CR4 notebooks ~18–19K LoC = biggest). KEEP: notebooks/file/ (local md viewer), drive/workflows/ arg-UI, generic editor crate, MCP, object_metadata/generic_string_objects sqlite tables.

#### NEXT — cloud_object removal TODO (resume here; each = one green 3-gate + `cargo build --features gui` commit)
Consumers → core ordering. Full keep-boundary + file:line map in memory `ai-strip-session-63-graphql-crate-deletion.md`. **KEEP throughout: cloud_object framework + warp_server_client (MCP store), `Workflow`/LocalWorkflows/browser, `notebooks/file/` (local md), `drive/workflows/` arg-UI, generic `editor` crate, MCP, `object_metadata`/`generic_string_objects`/`object_permissions` sqlite tables.**

- [x] **CR1** — remove cloud workflow search source (`0123abe3`). Deleted `cloud_workflows_data_source.rs`, reduced `WorkflowIdentity` → Local-only, removed registration. (`AcceptedWorkflow::Cloud` arm + `workspace/view.rs:10687` handler left for CR2 — now unreachable.)
- [x] **CR2** — remove WorkflowModal cloud create/edit subsystem (`0cac96ce`, −0.07 MB).
- [x] **CR3** — remove Env-Var Collections (`c9494999`). Deleted `app/src/env_vars/` (~5K LoC cloud-only) + entire pane + invocation flow + search + model persistence methods. KEPT slim `env_vars.rs` with `EnvVar`/`EnvVarValue`/`serialize_variables_for_shell` (used by subshell bootstrap). 3-gate 0/0/0.
- [x] **CR4** (`19b68ea1`, −4.41 MB) — cloud Notebooks subsystem removed. NotebookPane/Manager/CloudNotebook/Model/search/notebook_embedding/; KEPT notebooks/file/, editor/, link.rs.
- [x] **CR5** (multi-commit, net −4.95 MB) — CloudWorkflow/CloudWorkflowModel/WorkflowType::Cloud removed. CR5a `d2ee9d4a` WorkflowPane+Manager; CR5b-1 `58754147` behavioral collapse; CR5b-2 `f503949f` WorkflowView; finale `e5083582` type deletion.
- [x] **CR6** (`cd655243`, −0.82 MB) — Drive Folders + export/settings/helpers removed. Deleted `drive/folders/` (CloudFolder/FolderId), `drive/export.rs`, `drive/settings.rs`, `drive/drive_helpers.rs`. Removed UpsertFolder/UpsertFolders ModelEvents + upsert_folders + folder DB load block. Collapsed WarpDriveSettings guards (always-true). KEPT `DriveObjectType`/`WarpDriveItemId`/`OpenWarpDriveObjectArgs` (callers live), `cloud_object_styling.rs` (MCP needs styling icons), `CloudObjectTypeAndId` re-export.
- [x] **CR7** (`4f4743a6`, −0.28 MB) — persistence cleanup. Removed `LeafContents::Workflow` + `WorkflowPaneSnapshot` from app_state.rs; removed workflow_panes load/save/delete from sqlite.rs; collapsed ObjectIdType::Notebook/Workflow/Folder delete arms to no-ops. Also re-registered `NotebookKeybindings` singleton (dropped in CR4, caused startup panic). 3-gate 0/0/0.
- [x] **CR8** (`0ab5b848`, ~0 MB) — trimmed dead `cloud_object/mod.rs` helpers (object_link, breadcrumbs, semantic_permadeletion_countdown, get_top_folder_trashed_ts, lazy_static SPACE_DETECT_RE/SAFE_URL_CHAR_RE); also dead UpdateManager methods (session 66: fetch_single_cloud_object, replace_object_with_conflict, update_workflow_enum, duplicate_object, create_workflow_enum, trash_object, untrash_object + cascaded helpers); MCP still compiles. 3-gate 0/0/0.
...
- ~~`app/src/ai/document/` + `crates/ai/document.rs` stubs~~ ✅ DONE (`b5f15279`, −0.85 MB) — AIDocument/plans cluster removed (Part B).
...
- ~~conversation cluster (palette)~~ ✅ DONE (`40ea233d`, −0.25 MB) — conversations command-palette removed (Part A). NOTE: `conversation_types` (AIConversationId/ConversationStatus/ServerConversationToken) + `conversation_status_ui` KEPT — live (persistence/session-restore, CLI-agent status pills).

- **De-brand rename** (`warp*` → `sublight*`) — crate identifiers, `use warp_*` imports, remaining `warp.dev` / `warpify` string literals in UI, update `Cargo.toml` workspace member names. Binary name + Channel::Oss strings already done. Phone-home URL strip COMPLETE (s67). Remaining `server_root_url()` uses are config-definition + wasm-gated code only — not desktop phone-homes. Risk: mechanical fan-out across many files, no behavior change. Approach: recast + batch rename. Verify with `cargo check` after each crate rename.

- ~~warpify tail Cluster B~~ ✅ DONE (`111ca134`, session 67 start) — DProtoHook subshell/ssh parser + model-events + TmuxInstallationState all removed.
...
- [x] **R1a** (`187f2d24`) — severed `notebooks/link.rs` warp-drive web-link recognition; deleted `parse_url_paths.rs` + the dead `OpenWarpDriveLink` event chain + `cloud_object::extract_server_id_and_object_type_from_warp_drive_link`. Dropped the `server_root_domain` reader.
- [x] **R1b** (`11330061`, −249 LoC) — deleted the dead `issue_workload_token` chain + `WorkloadToken` + `IsolationPlatformError` + `docker_sandbox.rs` + namespace JWT + 7 unused deps (decoupled isolation_platform from warp_core). Kept `detect()`/`is_in_*`. Dropped the `workload_audience_url` reader.
- [x] **R1c** (`2569eeb5`, −344 LoC) — deleted the caller-less config cluster: WarpServerConfig/OzConfig (+::production() URL literals), Telemetry/RudderStack/Autoupdate/CrashReporting configs, all 14 accessors, derive_http_origin_from_ws_url, MOCK_SERVER statics, state_tests.rs, mockito dep. **app.warp.dev / rtc.app.warp.dev / sessions.app.warp.dev / Firebase API key / oz.warp.dev now absent from the binary** (grep-verified). Predicates → const false (configs were always None). −0.08 MB (`cf0e2c1c` size row).

### R2 — X-Warp fingerprint headers on ALL outbound HTTP ✅ DONE (`f2fbecec`, −0.04 MB)
`include_warp_http_headers()` returned `true` unconditionally → X-Warp-* on every request (MCP, OAuth, LSP downloads). Removed the `headers` module + `add_warp_http_headers` + `include_warp_http_headers` + the now-dead client-id reporting chain (`current_client_id`, `ExecutionMode::client_id`, `AppExecutionMode::client_id`, write-only `GLOBAL_EXECUTION_MODE`); simplified builder()/verb methods. The only live outbound-data leak from the audit — now closed. 3-gate 0/0/0.
- [x] R2 — strip warp headers from http_client
...
### R5 — AI/cloud FeatureFlag sweep (~23 variants) — IN PROGRESS
Census done (each flag's `.is_enabled()` guard count, excluding the warp_features enum def + the channel flag-lists).
- [x] **R5 batch-1** (`c7b3a875`) — 10 GUARDLESS flags (only listed in flag-enable lists, no `.is_enabled()` reader → enabling them was a no-op): AIRules, AIContextMenuEnabled, BlocklistMarkdownTableRendering, AgentManagementView, InteractiveConversationManagementView, LocalClaudeCodexChildHarnesses, SummarizationViaMessageReplacement, ConversationsAsContext, OrchestrationViewerPillBar, CustomInferenceEndpoints. Removed enum variant + all list entries (enabled_features/DOGFOOD/PREVIEW/RUNTIME) + flag_description arm + Cargo feature def + aggregate-list entry. RUNTIME_FEATURE_FLAGS now empty. 3-gate 0/0/0 + warp_cli/managed_secrets.
- [x] **R5 batch-2a** (`117173fd`) — 8 guarded flags, each collapsed to its flag-off branch then removed: AgentModeAnalytics (telemetry-disable term), CustomInferenceEndpointsEnterprise (enterprise-OR term), AIContextMenuCode (removed OutlineCodebaseSymbols settings widget+action; KEPT the live `outline_codebase_symbols_for_at_context_menu` setting read by ai/outline/native.rs), APIKeyAuthentication (api_key→always None), ProfilesDesignRevamp (model selector now driven by `show_model_selectors_in_prompt` setting alone), ArtifactCommand (artifact CLI subcommand always hidden/rejected), AgentModeWorkflows (dropped query-filter pushes), QueueSlashCommand (removed the whole `/queue` feature cascade — QUEUE static+slash-handler, ToggleQueueNextPrompt action decl/Display/classifier/handler, keybinding+const+re-exports). One cascade caught by gate-oracle: OutlineCodebaseSymbolsForAtContextMenuWidget. 3-gate 0/0/0.
- [~] **R5 batch-2b** — the 5 HIGH-FANOUT guarded flags (each on-by-default → gated code is currently live; collapse to off = real feature removal). 1 of 5 done:
  - [x] **OrchestrationV2** (`7c292598`) — removed `/orchestrate` slash cmd + ORCHESTRATE static + is_orchestration_enabled + parent-nav-card render chain (was already dead, card always None) + warp_cli run-message hide + 2 unit tests. 3-gate 0/0/0.
  - [x] **SharedWithMe** (`1f6eb155`) — removed Warp Drive object sharing (guests/anyone-with-link). Collapsed all guards to not-shared: persistence stops decoding/encoding sharing perms (guests/anyone_with_link always empty/None), workflows always editable, drive Shared-space dropped for users, owner_to_space→Personal. Cascaded out the CloudViewModel editability API (ContentEditability/object_editability/object_access_level), the decode_link_sharing re-export, the corrupted-guests test. KEPT generic cloud_object framework + CloudObjectGuest/SharingAccessLevel data types + serialization helpers (cfg(test)). 3-gate 0/0/0 + warp_server_client/warp_cli/managed_secrets.
  - [x] **DiffSetAsContext** (`6aab82ed`) — removed the "add diff SET as context" entry points (header standalone button, file-row add_context_button, 2 header-menu items, has_menu_flags term) + the now-dead AddDiffSetAsContext action/insert_diff_as_context + DiffSetContextScope + collapsed DiffSetScope→All-only. KEY: the header dropdown was gated `DiffSetAsContext && FileAndDiffSetComments` — re-gated to FileAndDiffSetComments alone so that KEPT feature keeps its dropdown. KEPT the always-on per-hunk gutter add-context button (separate `AddDiffHunkContext` path) + the generic code-editor add-context infra + context.rs + FileAndDiffSetComments. 3-gate 0/0/0.
  - [ ] **AgentView(17) + AgentMode(24) — NEEDS A PRODUCT DECISION, not a mechanical strip.** Investigation verdict: these are NOT dead-AI flags — they are **live UI-mode toggles**, both on-by-default, each with live `!flag` (classic-input) AND positive (agent-input) branches:
    - **AgentMode** switches the terminal between agent-mode input and classic input — gates `input:toggle_input_type` vs `workspace:toggle_ai_assistant`, keybinding `.with_enabled(|| AgentMode)` / `(|| !AgentMode)` pairs (init.rs:601/624/641/659), up-arrow history mode, search/command-palette items.
    - **AgentView** switches between the agent-view input UI (universal input + inline history menu + message bar + NLD) and classic input (`input/universal.rs` vs `input/classic.rs`).
    Collapsing either to `true` keeps the agent UI and deletes the classic-input fallback; collapsing to `false` does the reverse. **Which input UI should Sublight ship?** That's a product call the user must make. Until then, KEEP both (they're load-bearing for the surviving vendor-CLI agent input). Once decided, the collapse is a large two-sided cascade across terminal/input + editor + search + keybindings.
    HOANotifications/OpenCode/Codex/Gemini notification flags = vendor-CLI infra, KEEP.
    - **AgentMode teardown IN PROGRESS (session 70) — user chose FULL removal.** Increment 1/3 DONE (`3d2bbef2`): collapsed the cosmetic/low-cascade guards (editor ctrl-i→Inspect Command; search NaturalLanguage/WarpAi icons+color; command-palette AI suggested-action). Flag still defined; 3-gate 0/0/0. **REMAINING increment 2 (the deep cascade) + 3 (flag removal):**
      - Guards left (collapse to no-AI): command_search/zero_state.rs:284 (drop PromptHistory push — check QueryFilter::PromptHistory cascade), up_arrow.rs:98 (should_include_prompts→false; prompt-history code cascades dead), terminal/input.rs:1448 (delete `input:enable_auto_detection` AI binding → InputAction::EnableAutoDetection cascade), terminal/input.rs:2971 (agent_mode_hint_text placeholder → drop, keep else), terminal/view.rs:7699 (drop the "Attach as Agent Context" menu block) + 8212 (drop "Ask Warp AI" menu item) + 12790 (drop no-op `if AgentMode {}`), terminal/view/init.rs:601/624/641/659/670 (delete the WarpAi-group AI keybindings: Ask-Warp-AI / Attach-Selection-as-context), workflows/info_box.rs:675 (collapse AI-origin icon), workspace/mod.rs:886/953/1062 (delete toggle_ai_assistant + agent AI bindings), workspace/view.rs:9706.
      - Then the action cascade (becomes dead once all the above dispatchers go): `TerminalAction::ContextMenu(ContextMenuAction::AskAI)` + `AskAISource` enum + `ask_ai()` fn + `AttachSelectionAsAgentModeContext` custom action + `ATTACH_AS_AGENT_MODE_CONTEXT_TEXT` + `agent_mode_hint_text()` + the `BindingGroup::WarpAi` group + the `SearchItem::WarpAi` palette item.
      - Then the **input-mode** removal: `InputType::AI` arm + the universal-input `InputToggleMode::AgentMode` option (universal_developer_input.rs:146) → input bar shows Terminal/code only (the "A" button gone).
      - Then increment 3: remove the FeatureFlag::AgentMode variant + features.rs entry + Cargo `agent_mode` feature (+ check DOGFOOD/RELEASE lists).
      Use the gate-oracle to chase each cascade. Do as several green commits. This is the project's largest single strip — give it a focused session.
    - **VERIFIED (session 70): Agent Mode = Warp AI ONLY** (InputType::AI→ProtocolInputType::AI; `is_ai_input_enabled()` hardcoded `false` in ai/blocklist/mod.rs:44 → Agent Mode submit is a no-op now; zero AgentMode refs in vendor-CLI code; vendor CLIs use the separate `render_cli_agent_input` path in Terminal mode). So Agent Mode is a dead vestige. **BUT the AgentMode-removal is NOT a clean collapse:** its 24 guards fork between *new agent mode* (`AgentMode`) and the *legacy Warp AI Assistant* (`!AgentMode` → `workspace:toggle_ai_assistant` [workspace/mod.rs:880], the `WarpAi` command-palette item + `Icon::AiAssistant` [search_item.rs:185/212], command_palette SUGGESTED_ACTIONS [view.rs:53]) — BOTH dead Warp-AI UIs. Collapsing `→false` is compile-safe but *activates* the vestigial legacy-AI-assistant affordances (worse UX); `→true` keeps the dead agent UI. The correct strip removes BOTH AI sides across the 24 sites + the universal-input AgentMode toggle option (universal_developer_input.rs:146) + the legacy-assistant bindings — a careful dedicated effort, not a mechanical collapse. **This is the true keystone; do it as its own focused session.**
...
- **inc 2c step 6 DONE (session 62, `f8032df8`, −2.98 MB→653.9 MB):** THE KEYSTONE — `Team`/`TeamMember`/`TeamDeleteDisabledReason` structs + `Workspace.teams` field + current_team/has_teams/team_from_uid_across_all_workspaces/ai_allowed_for_current_team accessors + `#[cfg(test)] setup_test_workspace`/update_* helpers + SunsettedToBuildDataUpdated event(0-sub) + cli_agent Uber-team special-case(dropped `ctx` from now-pure `detect`) + gql `From<GqlTeamMember>` + persistence team save/load(app-side; KEPT team_* tables) — deleted as ONE atomic commit (the previously-reverted sprawl). Team-behavior/policy-override tests deleted; no-team default tests kept. team.rs reduced to `MembershipRole`. 3-gate 0/0/0 + warp_cli/managed_secrets clean. **KEY: the uid-carrying `::Team` ENUM VARIANTS (Space/Owner/WorkflowSource/NotebookLocation/TelemetrySpace) are INDEPENDENT of the struct (carry `team_uid: ServerId`, not `Team`) → left for a separate cut. `TeamTesterStatus` is NOT teams — it's the misnamed live data-poller trigger (subscribed by both update managers); KEPT.**
- **inc 2c step 5/6 remainder ATTEMPTED → REVERTED (session 61) → then DONE session 62:** the s61 attempt sprawled into team-behavior test surgery; reverted to `6d6ae1c1`. **LESSON: field/struct removal is NOT incrementally separable from its test suite — landed in s62 as ONE prepared atomic commit (field+struct+persistence+all dependent test files together), gated once.**
- **inc 2c steps 1/2/4/6a DONE (`8cddbfa1`→`ffeaa335`):** step1 (`e483acec`) tier-limit/capacity cluster + team_from_uid; step2 (`c9193c09`) accessor-consumer collapses (mcp/upgrade/rate-limit/workflow-categories/notebook-menu) + current_team_uid/mut; step4 (`39d889f6`) ~18 policy accessors → no-team defaults (current_team now 0 production callers) + team_spaces; **step 6a (`ffeaa335`) = THE BEHAVIORAL CUT: gql `From<GqlWorkspace>` sets `teams: Vec::new()` ignoring the cynic teams array → NO teams even when API-key-authed. GOAL MET — teams never observed in production.** Remaining = pure dead-code/variant/persistence/test hygiene (cross-crate `Owner::Team` in warp_server_client + diesel; KEEP team_* tables + MembershipRole + cynic types) — full 5-phase map in `ai-strip-session-61-teams` memory.
- **inc 2c step 7 DONE (session 62, `5242b774`, −0.02 MB — linker-near-dead):** the app-side `::Team` enum-variant cluster. Removed `Space::Team`/`WorkflowSource::Team`/`NotebookLocation::Team`/`TelemetrySpace::Team` (verified serde-vestigial: zero `persistence/` deser sites → deser-safe). Collapsed all match arms + `From<Owner>`/`From<Space>` conversions to personal/shared fallback (`owner_to_space` Owner::Team→Space::Shared, From<Owner>→PersonalCloud); neutered MCP team-share (`share_templatable_mcp_server`→no-op, deleted `get_first_team_space_id`); dropped the dead can't-move-team-object guard; deleted the Space::Team embedding test. **KEPT (schema/DB/wire-bound, cross-crate — untouched): warp_server_client `Owner::Team` (cynic `SpaceType` try_from boundary + local-DB "TEAM" serde), `ServerGuestSubject::Team`/`Subject::Team`/`TeamKind`, managed_secrets `SecretOwner::Team`, cynic graphql team types.** Owner::Team still arises from gql/DB deser and now maps to Shared everywhere downstream. 3-gate 0/0/0. **TEAMS STRIP FULLY COMPLETE — feature + data model + app variants all gone; only schema/wire-bound Owner::Team boundary remains (correctly, by the KEEP rule).**
- **~~REMAINING (teams) = the `::Team` enum-variant cluster~~ (DONE above, step 7):** [superseded] — `Space::Team`(cloud_object/mod.rs:1126) + `Owner::Team`(crates/warp_server_client/src/cloud_object/mod.rs:322 — CROSS-CRATE) + derived `WorkflowSource::Team`/`NotebookLocation::Team`/`TelemetrySpace::Team`/`SecretOwner::Team`(managed_secrets) + `Container::Team`/`Subject::Team`/`TeamKind`(warp_server_client). These carry only `team_uid: ServerId` (never the `Team` struct, now deleted). Match arms at: ai/mcp/templatable_manager/native.rs:412/1438/1496, cloud_object/mod.rs:1136/1155/1167, cloud_object/model/{persistence:188,view:192}, notebooks/{mod:118,notebook:1701}, search/notebook_embedding/mod.rs:18/22/28, server/telemetry/events.rs:45, terminal/history.rs:229, workflows/workflow_view.rs:847/854, workspace/view.rs:11629/14874/15477, user_workspaces.rs:289/311/313(space_to_owner/owner_to_space), persistence/sqlite.rs:2974("TEAM"→Owner::Team deser). **TRAPS for this cut:** (a) warp_server_client Owner/Container/Subject + `From`s are cynic-graphql-schema-bound (`SpaceType::Team`/`OwnerType::Team`/`GuestSubject::TeamGuest` at :999/1047/1066) — likely map→fallback, NOT delete, like the LLMModelHost wire variant; (b) `SecretOwner::Team` (managed_secrets) check schema-bound; (c) cross-crate → gate `-p warp_server_client` + `-p managed_secrets`; (d) collapsing owner_to_space's `Owner::Team` branch (currently `!SharedWithMe → Space::Team{uid} else Space::Shared`) changes drive-space mapping — verify no live object is ever team-owned (teams never populate, so safe). Lower priority — these are dead-but-compiled tags, not a live feature. **The teams FEATURE removal is COMPLETE; this is residual variant hygiene.**
...
1. **AIClient** (safest, AI already gutted) — remove consumers feature-by-feature then trait+impl+get_ai_client+the gql ops. PROGRESS:
   - **(1d DONE `1bcfd4e2`)** connected_self_hosted_workers (dead scaffolding) + cancel_ambient_agent_task (0-caller). −185 LoC.
   - **(1c DONE `90221ead`, −545 LoC)** workflow AI "Autofill" (generate_metadata_for_command): removed from WorkflowView (`7c6f1985`) + WorkflowModal + deleted ai_assist.rs + the trait method + GenerateMetadataForCommand gql op + workspace/view ctor cascade + tests. **Side effect: AIRequestUsageModel::refresh_request_usage_async + get_request_limit_info are now dead (their only callers were the AI-assist handlers).**
   - **(1b DONE `f83e4757`, −0.41 MB)** code_review AI (generate_code_review_content): open-time AI commit msg (commit.rs) + AI PR title/body (pr.rs) → manual path; deleted the trait method + impl + ai/generate_code_review_content/ module. **AIClient now has ONLY get_request_limit_info (dead since 1c).**
   - **(1a NEXT — DECISION MADE: REMOVE ALL WARP-AI SURFACES)** User chose full removal (no Warp agent at all; vendor CLI agents as shell processes are untouched). So this is a multi-increment epic, not one cut. Scope (each its own green increment; do the render/input-path ones with a GUI build + eyeball, cargo-green ≠ works):
     - **(1a-i DONE `38c35558`)** feedback skill via Warp agent removed — send_feedback always opens the form URL; deleted is_feedback_skill_available + menu label + slash-cmd /feedback gate.
     - **(1a-ii DONE `38c35558`)** code_review: removed `ReviewDestination::Warp` (send-to-Warp-agent) — review comments go only to CLI agents; send button drops ai_available gating.
     - **(1a-iii DONE `6efefa8c`)** terminal — turned out to be just 2 repaint subscriptions (ctx.notify on usage change) in input.rs + universal_developer_input.rs, NOT the agent-mode input. Removed both + imports. (Agent-mode input itself = the BlocklistAIInputModel/InputType subsystem, a separate later concern.)
     - **(1a-iv-a DONE `70a1346a`)** dropped the last has_any_ai_remaining caller — comment_list_view::debug_state no longer reads the model; removed ai_available/ai_enabled from CommentListDebugState + right_panel debug log. **has_any_ai_remaining now 0 prod callers.**
     - **(1a-iv-b NEXT — model + RequestLimitInfo/BonusGrant deletion, WIDE cascade; cargo-oracle it)** Delete AIRequestUsageModel + RequestUsageInfo + RequestLimitInfo + BonusGrant + BonusGrantScope + RequestLimitRefreshDuration + the model's methods from `ai/request_usage_model.rs`. Cascade: lib.rs:203/1127 (import+reg+`ai_client` local), test_util/terminal.rs:20/73, request_usage_model_tests.rs (~25 tests), settings/ai_tests.rs quota tests, settings/ai.rs (update_quota_info[0-prod-caller, dead]/should_display_quota_reset_banner[0-prod-caller]/mark_quota_banner_as_dismissed[2 prod callers: context_chips/display_chip.rs:771 + terminal/input.rs:7780 — collapse those]), gql_convert.rs (BonusGrant::from_gql_bonus_grant + BonusGrantScope + bonus-grants-purchased conversion — verify from_gql_bonus_grant callers), server_api/ai.rs get_request_limit_info → 1e. **KEEP-NUANCE (schema-bound): `BannerState` + `CycleInfo` in settings/ai.rs are `settings_value::SettingsValue` (Serialize/Deserialize/JsonSchema) = PERSISTED user settings — do NOT delete the structs (settings-migration); only remove the now-dead methods that operate on them.** Use the cargo-oracle (delete model + get_request_limit_info, read the break list, cascade).
   - **(1a-iv-b + 1e DONE `0bbb8725`, −1404 LoC)** Deleted ai/request_usage_model.rs (AIRequestUsageModel + RequestUsageInfo + RequestLimitInfo + BonusGrant + BonusGrantScope + RequestLimitRefreshDuration + AIRequestUsageModelEvent) + ~25 model tests + the 6 test-file registrations + lib reg/ai_client + settings update_quota_info/should_display_quota_reset_banner + quota tests + gql_convert from_gql_bonus_grant. **1e: AIClient trait + impl + ServerApiProvider::get_ai_client all deleted — AIClient is GONE.** KEPT (settings-persisted): BannerState/CycleInfo/AIRequestQuotaInfo + mark_quota_banner_as_dismissed. KEPT (schema): GetRequestLimitInfo cynic gql op in warp_graphql. 3-gate 0/0/0 + warp_cli clean.
   - **AICLIENT TRAIT FULLY REMOVED.** Remaining graphql blocker traits: ManagedSecretsClient, TeamClient/workspaces, AuthClient, then crate deletion.
2. **ManagedSecretsClient DONE (`632b0c48`, −5.88 MB → 645.7 MB)** — was dead scaffolding (ManagedSecretManager registered but 0 readers, all methods 0-caller). Deleted server_api/managed_secrets.rs (7 cynic gql op-builders) + registration + getter. KEPT warp_managed_secrets crate (ManagedSecretValue → MCP). Big delta from cynic op-builder monomorphizations.
3. **TeamClient::workspaces_metadata DONE (`7aa7060d`, −6.35 MB → 639.4 MB)** — reverted the first sprawling attempt, then applied the mapped edit-set atomically + GUI-launch-verified. Deleted update_manager.rs/TeamClient/WorkspacesMetadata types/on_workspaces_updated/From<GqlUser>/5 test regs. UserWorkspaces now sqlite-cached-only. KEPT unused gql_convert From<Gql*> (compile fine, fall with the crate).
4. **AuthClient + crate-deletion FINALE — decided REMOVE remote sessions (fully local); fully scoped, fresh GUI+remote-verify session.** ServerApi egress ALL DEAD (send_graphql_request=test-only; stream_agent_events/get_relevant_files/generate_multi_agent_output/notify_login/server_time/fetch_channel_versions/get_or_create_ambient_workload_token=0-caller). KEEP get_http_client (LSP/init_project). AuthClient used only by remote-server auth (build_auth_context→RemoteServerAuthContext, remote_server_controller.rs:327 + remote/auth_context.rs) + dev CopyAccessTokenToClipboard. Steps: (a) delete dead egress methods + send_graphql_request test block + get/post_public_api helpers; (b) delete AuthClient + get_auth_client + remote-server auth path + dev action; (c) **CRATE-DEP ANALYSIS** — does kept local drive/cloud_object code use warp_server_client Owner/Space/Subject? does anything use warp_graphql beyond dead gql_convert (PricingInfo/billing/Workspace-gql in local Workspace)? likely a partial trim not clean 3-crate rm; (d) delete warp_graphql + warp_graphql_schema (+ warp_server_client if remote fully gone) + dead gql_convert — schema-bound Owner::Team/cynic SpaceType/OwnerType fall here. Full detail in `ai-strip-session-62-graphql-removal` memory.
