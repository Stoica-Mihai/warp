...
- [~] **R5 batch-2b** — the 5 HIGH-FANOUT guarded flags (each on-by-default → gated code is currently live; collapse to off = real feature removal). 1 of 5 done:
  ...
  - [ ] **DiffSetAsContext(7)** — DEEP CASCADE: "add diff set as AI context" UI woven through code_review header buttons (render_add_diff_set_context_button + the DiffSetAsContext&&FileAndDiffSetComments dropdown), file-row `add_context_button`, two header menus (header_menu_items / header_menu_items_new), the `CodeReviewAction::AddDiffSetAsContext`/`DiffSetScope` action. Collapse to off, chase the cascade. KEEP FileAndDiffSetComments (separate flag) + the generic code-review pane.
  - [ ] **SharedWithMe(12)**, **AgentView(17)**, **AgentMode(24 — keystone AI master-switch; determine on/off FIRST; collapsing cascades enormously across the whole agent surface)**. Do AgentMode LAST. HOANotifications/OpenCode/Codex/Gemini notification flags = vendor-CLI infra, KEEP.
