...
- **De-brand rename** (`warp*` → `sublight*`) — crate identifiers, `use warp_*` imports, remaining `warp.dev` / `warpify` string literals in UI, update `Cargo.toml` workspace member names. Binary name + Channel::Oss strings already done. Remaining `ChannelState::server_root_url()` callers in `uri/web_intent_parser.rs` (URL pattern matching, not a phone-home) + toast at `workspace/view.rs:13487` + CLI override in `lib.rs` — these are NOT phone-homes (no outbound calls), can be addressed in the de-brand pass. Risk: mechanical fan-out across many files, no behavior change. Approach: recast + batch rename. Verify with `cargo check` after each crate rename.
