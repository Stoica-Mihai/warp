...
**Binary**: 710.6 MB (−8.15 MB session 57 codebase-indexing strip). 3-gate **111/82/112** (0/0/0 errors; +3 vs 108/79/109 = generic dead code exposed + local_fs-gated `create_agent_task`, KEEP).
...
- (2) ⬜ BACKEND (next, cross-crate — do as ONE green increment): app/src/ai/aws_credentials.rs (whole, 416 LoC, OIDC+STS+local-chain refresh) + aws_credentials_tests.rs; lib.rs:152 import + :1147 `subscribe_to_settings_changes` call; terminal_manager.rs:29 import + :188 `register_model_event_dispatcher` call; crates/ai/api_keys.rs AWS surface (AwsCredentials/AwsCredentialsState re-export L7, AwsCredentialsRefreshStrategy enum L80-94, `aws_credentials_state`+`aws_credentials_refresh_strategy` fields L99-100 + init L109-110, 4 set/get accessors L219-240, `include_aws_bedrock_credentials` param + include_aws OIDC logic L295-344); crates/ai/aws_credentials.rs (whole, data types). **CAUTION:** `ManagedSecretManager::issue_task_identity_token` (managed_secrets/manager.rs:121-126 + used at :151 inside get_task_secrets?) — verify whether manager.rs:151 is a Bedrock-only path before trimming; KEEP the generic manager + get_task_secrets/list_secrets + `ManagedSecretsClient` + server_api/managed_secrets.rs. The issue_task_identity_token graphql op + the ManagedSecretsClient::issue_task_identity_token trait method are Bedrock-only → removable.
- (3) ⬜ SETTINGS: settings/ai.rs 5 aws_bedrock_* settings (note `aws_bedrock_auto_login` is ALREADY unread after banner removal) + AISettingsChangedEvent::{AwsBedrockProfile,AwsBedrockAuthRefreshCommand,AwsBedrockCredentialsEnabled} variants; aws_bedrock_login_banner_dismissed also read by workspace/view.rs + workspace/mod.rs (DebugReset) + session_state.rs — remove those together.
- (4) ⬜ WORKSPACE: LLMModelHost::AwsBedrock variant (workspace.rs:749) + user_workspaces 5 accessors (491-535) + gql_convert + the 2 graphql enums (crates/graphql workspace.rs:287 + get_feature_model_choices.rs:72) + user_workspaces_tests.
- (5) ⬜ CRATES + warp_cli: managed_secrets 2 Bedrock ManagedSecretValue variants (AnthropicBedrockAccessKey/ApiKey + constructors) + issue_task_identity_token op; warp_cli secret.rs Bedrock subcommands + agent.rs bedrock args + lib_tests. Gate incl `-p warp_cli`.
Gate incl `-p warp_cli` from increment 2 on. KEEP ManagedSecretValue type + ManagedSecretManager (generic, MCP). Expect real shrink (OIDC + aws-sdk-sts + graphql ops live-linked).
