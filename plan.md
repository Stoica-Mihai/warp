...
| Telemetry — remaining | ⬜ todo | — | (a) ✅ DONE — **16/16 satellite `*TelemetryEvent` enums removed** (batch 1 `e466235a` 4 pure, batch 2 `2eebd0fe` 9 mixed surgical, batch 3 `8921cfcb` 3 crate-level: ai/onboarding/repo_metadata). Helper types kept (events.rs pattern). All event construction was in no-op'd macro args; only re-export chains + builder fns needed fixing. **Batch 1 (`e466235a`)**: 4 pure ones removed (Antivirus, Settings, Cli/agent_sdk, ThirdPartyHarness) — each had tentacles (builder fn `command_to_telemetry_event`, tests, construction blocks). **Remaining ~12**: 9 are MIXED — their `telemetry.rs` also defines helper enums used by real code (`SkillOpenOrigin`, `CodeReviewPaneEntrypoint`, `VerticalTabsDisplayOption`, `OpenedFrom`, `Lsp*Source`, `CloudModeEntryPoint`, `TabConfig*`, blocklist `Orchestration*`/`TeamAgent*`, agent_management `SetupGuideStep`/`ArtifactType`/`FilterType`) → surgical: delete only the event enum + impls + `register_telemetry_event!`, KEEP helper types (like events.rs). + 3 crate-level (`crates/ai`, `crates/onboarding`, `crates/repo_metadata`) + the `warp_core` trait. (b) warpui/warp_core record layer (`record_event`, `record_telemetry_*` macros) + `EnablementState`/`TelemetryEvent` trait. (c) dispatch infra files (`collector`/`rudder_message`/`TelemetryApi`) — coupled to bootstrap flush/persist timer. (d) bootstrap wiring in `lib.rs`. (e) ✅ partial (`bcd1fc6a`): `cargo fix --features gui,local_fs` swept unused imports/vars **795 → ~142**; remaining ~142 = `dead_code` on never-constructed payload structs in `events.rs` — die with (a)/(b). Lesson: `cargo fix` under one feature set drops `#[cfg(test)]`/other-feature-only imports → re-verify all 4 gates after (it broke `--tests`, fixed by restoring 1 re-export). |
