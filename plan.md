...
### AI strip — current state (2026-06-08 session 63)

**Binary**: **629.7 MB** (`0123abe3`). 3-gate **0/0/0 errors** throughout. Per-session detail lives in the `ai-strip-session-*` memory files (this plan stays high-level for 57+).
...
**S63 NEW WORK — cloud_object FEATURE removal (started).** Decision after offline-usability investigation: keep ONLY **LocalWorkflows** (genuinely usable: `ctrl-r`/`input:toggle_workflows` → built-in + `~/.warp/workflows` YAML → insert command); REMOVE the 4 create-gated-dead/broken Drive features (Notebooks, CloudWorkflows, Env-Var Collections, Drive Folders). **CORRECTIVE: warp_server_client + cloud_object framework STAY — MCP servers persist through them; remove the object TYPES, keep the framework.** 8 ordered increments CR1–CR8 (tasks + full map in memory). **CR1 DONE** (`0123abe3`): cloud workflow search source removed, WorkflowIdentity reduced to Local. CR2–CR8 remain (CR4 notebooks ~18–19K LoC = biggest). KEEP: notebooks/file/ (local md viewer), drive/workflows/ arg-UI, generic editor crate, MCP, object_metadata/generic_string_objects sqlite tables.
...
- [ ] **CR2** — remove cloud workflow create/edit modal. Delete `drive/workflows/modal.rs`(+tests) + `enum_creation_dialog.rs`; gut `OpenWorkflowModal*` actions (`terminal/view.rs`) + `build_workflow_modal`/`open_workflow_with_*` (`workspace/view.rs`) + `AcceptedWorkflow::Cloud` (searcher.rs + view.rs:10687). KEEP `arguments.rs`/`workflow_arg_selector.rs`/`workflow_arg_type_helpers.rs` (local arg UI). Verify local workflow run still works.
- [ ] **CR3** — remove Env-Var Collections. Delete `app/src/env_vars/` entire (~5K, only `Cloud` variant — no local env-var feature) + `EnvVarCollectionPane`/`invoke_environment_variables`/`AcceptEnvVarCollection`/`create_env_var_collection`/`CloudEnvVarCollectionModel`/`JsonObjectType::EnvVarCollection` + its sqlite deser arm.
- [ ] **CR4** (BIGGEST ~18–19K) — remove cloud Notebooks. Delete `notebooks/editor/`+`notebook/`+`manager.rs`+`notebook.rs`+`active_notebook_data.rs`+`link.rs`+`context_menu.rs`+`CloudNotebookModel`+`notebook_embedding/`+`search/notebook_embedding/`. **KEEP `notebooks/file/`** (FileNotebookView local md) +`styles.rs`. Remove `NotebookPane`/`create_notebook`/`open_notebook`/`AcceptNotebook`; KEEP `LocalFileNotebook` snapshot + `open_file_notebook` + `ReplaceWithFilePane`. KEEP generic `editor` crate. Split editor-vs-model sub-steps.
- [ ] **CR5** — remove CloudWorkflow wrapper + cloud WorkflowManager + WorkflowEnum decouple. Delete `workflows/manager.rs`(cloud)+`export_workflow.rs`+`WorkflowType::Cloud/Notebook` arms+`CloudWorkflowModel`+`CloudWorkflow`; decouple `WorkflowEnum`/`CloudWorkflowEnum` from kept arg-selector; remove `WorkflowPane`; reduce `WorkflowType` → `Local`/`AIGenerated`. KEEP `workflow.rs`/`local_workflows.rs`/`categories.rs`/`workflow_view.rs`/`info_box.rs`/`aliases.rs`/`command_parser.rs`.
- [ ] **CR6** — remove Drive Folders + drive index/export. Delete `drive/folders/`+`drive/export.rs`+`drive/settings.rs`+`drive/cloud_object_styling.rs`+`DriveIndexVariant`/`DriveObjectType`/`DriveSortOrder`/`WarpDriveItemId` enums. KEEP the `CloudObjectTypeAndId` re-export MCP needs.
- [ ] **CR7** — persistence cleanup. Remove `UpsertWorkflows`/`UpsertNotebooks`/`UpsertFolders` ModelEvents + `upsert_*` fns + `notebooks`/`workflows`/`folders` diesel schema refs. KEEP `object_metadata`/`generic_string_objects`/`object_permissions` (MCP). No down-migrations (permanent fork).
- [ ] **CR8** (optional) — trim now-dead `cloud_object/mod.rs` folder/space/trash/breadcrumb helpers only the removed Drive types used; re-verify MCP compiles.
