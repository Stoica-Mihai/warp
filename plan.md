...
### Frontier A — Codebase-indexing / embeddings (RECOMMENDED next; clean, clearly-cloud)
**Verdict: ALIVE + cloud-coupled (sends code fragments + merkle hashes to Warp GraphQL servers).** Off by default (`auto_indexing_enabled=false`, behind cargo feature `full_source_code_embedding`). Isolated subtree with a clean `StoreClient` trait seam at the cloud boundary — its only non-test consumer outside the `ai` crate is `server_api/ai.rs`. Spans 3 crates (`warp`+`ai`+`graphql`); `cargo check -p warp` covers them as deps.
