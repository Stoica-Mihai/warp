...
- **AMBIENT AGENTS strip — IN PROGRESS (session 67).** Warp cloud scheduled/cron AI agents. **Creation is RUNTIME-DEAD** (every gate hardwired false/None: `is_ambient_agent_session()→false`, `ambient_agent_task_id()→None`, `SessionSourceType::AmbientAgent` 0 constructors, `PanesLayout::AmbientAgent` never built). DONE: `github_auth_notifier` (`f19dcccd`) + terminal_pane snapshot collapse; **ambient cloud DATA LAYER (`7152c906`, −0.26 MB)** — `scheduled.rs`/`task.rs`/`spawn*.rs` deleted + `JsonObjectType::{ScheduledAmbientAgent,CloudAgentConfig}` variants + sqlite load arms gone; `ambient_agents/mod.rs` now holds ONLY `AmbientAgentTaskId`. **REMAINING (full file:line map in `ai-strip-session-67-audit-cleanup.md` memory):**
  - **(environments UI sub-feature, ~107 refs in display_menu.rs — its own focused atomic commit)** Delete `app/src/ai/cloud_environments/` + `JsonObjectType::CloudEnvironment` variant + sqlite CloudEnvironment load arm (`sqlite.rs:53-55` import + `:2318-2332` arm) + the WHOLE `ChipMenuType::Environments` surface in `context_chips/display_menu.rs`: the enum variant (`:87`) + ~25 match arms (`:244,253,262,271,289,294,320,943,960,971,1071,1124,1132,1158,1250,1292,1348,1411`) + the sidecar machinery (`EnvironmentSidecarData` struct `:160`, `EnvironmentSidecarSide` `:168`, `should_show_environment_sidecar`/`environment_sidecar_{data,anchor_id,side,positioning,overlay}`/`render_environment_sidecar` `:622-840`, render call `:1430-1431`) + `DisplayChipMenuAction::CopyEnvironmentSidecarField` variant `:237` + handler `:1474` + ENV_* sizing constants. The Environments menu is NEVER constructed (only a self `==` check at `:623`) → all arms dead. Mechanical but large; do as one careful pass.
  - **(core, HARD atomic w/ tests)** `AmbientAgentTaskId` plumbing + `LeafContents::AmbientAgent`/`AmbientAgentPaneSnapshot` (app_state) + pane-creation chain (pane_group: create_cloud_mode_terminal/create_ambient_agent_terminal/initial_ambient_agent_pane/`PanesLayout::AmbientAgent`/restore arm/`PaneMode::Cloud`) + terminal_model `is_shared_ambient_agent_session`/`ambient_agent_task_id` + `ConversationTranscriptViewerStatus::ViewingAmbientConversation` variant (KEEP Loading/ViewingLocalConversation — generic) + workspace dead actions `OpenOrAttachAmbientAgentConversation`/`OpenConversationTranscriptViewer` + handlers (view.rs:14344-14383, never dispatched) + `find_pane_with_ambient_agent_conversation`/`ambient_agent_task_id_for_*` + drop `Option<AmbientAgentTaskId>` params from `load_cloud_conversation_into_new_transcript_viewer`/`notify_terminal_focus_change` + `Indicator::AmbientAgent` (tab.rs) + `TabConfigPaneType::Cloud`/`PaneMode::Cloud` + `terminal/view/ambient_agent/` dir + persistence sqlite ambient_agent_panes load/save + delete `ai/ambient_agents/` dir. **KEEP (Frontier-B, not ambient):** `open_cloud_conversation_from_server_token`, `new_for_conversation_transcript_viewer_loading`, ServerConversationToken/AIConversationId paths. **Persistence: LEAVE the `ambient_agent_panes` sqlite table + migration** (old rows degrade gracefully via existing `bail!("Unrecognized pane kind")`). Test files to edit IN the same commit: `tab_configs/session_config_tests.rs`, `pane_group/mod_tests.rs`, `terminal/input_tests.rs`, `test_util/terminal.rs`, `workspace/view_tests.rs`, `ai/cloud_environments/mod_tests.rs`.
...
- **warpify tail Cluster B** (deferred — no warnings, no payoff, live SSH edges) — the ANSI OSC/DCS subshell/ssh parser (`model/ansi/dcs_hooks.rs` `DProtoHook::{InitSubshell,InitSsh,SshTmuxInstaller,TmuxInstallFailed,RemoteWarpificationIsUnavailable}`) + the model-events (`InitSubshell`/`InitSsh`/`SourcedRcFileInSubshell`/`SshTmuxInstaller`/`TmuxInstallFailed`/`RemoteWarpificationIsUnavailable`/`TmuxControlModeReady`/`DetectedEndOfSshLogin`) + `TmuxInstallationState`. Fully plumbed (parser→event→ModelEvent→no-op view handler); no warnings. Live edges: `TmuxControlModeReady`→`pty_controller.rs:159`, `DetectedEndOfSshLogin` drives SSH-login-detection in terminal_model.rs. **KEEP `SubshellSource`** (warpify/mod.rs — drives generic subshell flag, NOT dead). Defer to ssh-testable session; see `ai-strip-session-60-warpify.md` + `ai-strip-session-61.md`.

- **Dead warp:// deep-link URI arms** (cosmetic, optional) — `ObjectType::Folder/Notebook` match arms in `root_view.rs:1373` and `workspace/view.rs:8768` handle cloud drive URLs that can't lead anywhere since folders/notebooks are deleted. No warnings (compiler sees live match arms), no binary reduction. Collapse to `_ => warn!` or remove — safe but low-value.
