...
3. **TeamClient::workspaces_metadata** + TeamUpdateManager poll — ATTEMPTED session 62 → REVERTED (sprawled). **MAPPED EXACT SCOPE (prepared edit set for a fresh GUI-verified session):** the cut is much bigger than "a trait" — removing the `workspaces_metadata` fetch makes the ENTIRE workspace gql→app conversion layer dead, because `GqlUser`/`GqlWorkspace` have ZERO callers outside `workspaces/gql_convert.rs`. Full set: delete `workspaces/update_manager.rs` (TeamUpdateManager) + lib.rs reg/import + auth/mod.rs:180 stop-poll block + settings_view/code_page.rs:1418 on_page_selected refresh + test_util + **5 test files** (pane_group/mod_tests, workspaces/user_workspaces_tests [also imports server_api::team], workspace/view_tests, terminal/input_tests, search/.../view_tests — all register `TeamUpdateManager::mock`); delete `server_api/team.rs` (TeamClient trait+impl) + server_api.rs mod/import/get_team_client; delete user_workspaces.rs `WorkspacesMetadataResponse`+`WorkspacesMetadataWithPricing`+`on_workspaces_updated`(0-caller)+feature_model_choices+FeatureModelChoice/PricingInfoModel/Result/report_error imports; **delete ~all of `workspaces/gql_convert.rs`** (From<GqlUser>/From<GqlWorkspace> + every dependent From impl — the whole ~900-line file is fed only by the fetch; verify each From has no other caller) + the get_workspaces_metadata_for_user gql op; lib.rs drop the now-unused `server_api_provider` let-binding (keep the registration — auth/http still need it); `get_request_context` re-export may go 0-caller. PricingInfoModel never updated after (reads defaults — keep or remove). **REQUIRES GUI launch-verification: the app must still start + function with UserWorkspaces never populated (always local-empty).** This + AuthClient (trait 4) are the auth/workspace runtime foundation — do together in a fresh session with `cargo build --bin sublight --features gui` + launch.
