...
- **inc 2c step 5/6 remainder ATTEMPTED → REVERTED (session 61):** tried deleting `Workspace.teams`+`Team` struct+persistence app-side+variant arms; sprawled into ~4 test files of team-behavior tests (build Team{}/assert has_teams/TEST_TEAM); reverted to green `6d6ae1c1`. **LESSON: field/struct removal is NOT incrementally separable from the test suite — must land field+struct+persistence+all dependent test files+variants as ONE atomic commit, gated once. The behavioral cut (below) WAS separable + is done green.**
