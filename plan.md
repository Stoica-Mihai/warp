...
- [ ] **R5 batch-2b** — the 5 HIGH-FANOUT guarded flags, each its own mini-project: OrchestrationV2(7), DiffSetAsContext(7), SharedWithMe(12), AgentView(17), **AgentMode(24 — likely the keystone AI master-switch; determine on/off in this fork FIRST, then collapse — huge cascade)**. AgentView gates surviving agent UI. Do AgentMode last; the cascade for each is large. HOANotifications/OpenCode/Codex/Gemini notification flags = vendor-CLI infra, KEEP.
